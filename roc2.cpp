...
int main(int argc,char ** argv)
{
	...
	desc.add_options()
		("help,h","打印当前使用方法")
		("input,i",value<string>(&inputdir),"cropped LFW文件夹路径")
	...
	for(vector<boost::tuple<string,string,bool> >::iterator it = list.begin() ; it != list.end() ; it++) {
		...
		TensorCUDA input1 = TensorCUDA(preProcess(img1));
		...
		TensorCUDA input2 = TensorCUDA(preProcess(img2));
...
TensorCPU preProcess(Mat img)
{
	assert(150 == img.cols);
	assert(150 == img.rows);
	vector<TIndex> dims({1,3,img.rows,img.cols});
	vector<float> data;
	
	for(int c = 0 ; c < 3 ; c++)
		for(int h = 0 ; h < img.rows ; h++)
			for(int w = 0 ; w < img.cols ; w++) {
				data.push_back(img.ptr<unsigned char>(h)[w * 3 + c]);
			}
	assert(data.size() == 1 * 3 * img.rows * img.cols);
